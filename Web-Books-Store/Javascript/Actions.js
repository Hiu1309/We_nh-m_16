  ...
  if (webLogo) {
    webLogo.forEach((element) => {
      ...
      element.addEventListener("click", () =>
        location.replace(`${location.origin}${originPath}`)
      );
    });
  }
}
...
function slidesNews(elementsObj) {
  ...
  nextButtons.addEventListener(
    ...
    bridge.throttle(
      () => {
        if (++slidesIndex > container.length) slidesIndex = 1;
  ...
  prevButtons.addEventListener(
    "click",
    bridge.throttle(
      () => {
        if (--slidesIndex < 1) slidesIndex = container.length;
        showSlides(container, slidesIndex, 3);
      },
      200,
      "nextBtn"
...
function scrollToHandler(nameStaticPage) {
  ...
  if (nameStaticPage === "news") staticPage = elementsObj.getNewsBlogs();
  ...
  if (!staticPage && nameStaticPage === "news") {
    ...
    window.history.pushState(
      {},
      "",
      `${nowPath.slice(0, nowPath.lastIndexOf("/HTML/") + 6)}index.html`
    );
...
function staticContents(elementsObj) {
  ...
  if (scrollTopButtons) {
    scrollTopButtons.addEventListener(
      "click",
      bridge.throttle(() => scrollToHandler("scrollTop"), 200, "ScrollTopBtn")
    );
  }
}
        ...
        if (pathsObj[path]) {
          switch (path) {
            ...
            case "/account/forgot_password":
              if (path === "/account/") path = "/account/login";
              renderDOMHandler(
                "account",
                `${path.slice(path.lastIndexOf("/") + 1, path.length + 1)}`
              );
              break;
            ...
            case "/order/history":
              ...
              else renderDOMHandler("orderHistory");
              break;
...
function checkActiveHTML(nameForm, ...restForms) {
  ...
  if (!nameForm.classList.contains("active")) nameForm.classList.add("active");
  return nameForm;
}
...
function accountEvents(elementsObj) {
  ...
  if (!loginButtons || !registerButtons || !forgotButtons) return false;
...
async function renderDOMHandler(nameDOM, ...requestRests) {
  try {
    const elementsObj = bridge.default();
    const originPath = location.pathname.slice(
      0,
      location.pathname.lastIndexOf("/HTML/") + 5
    );
    ...
    if (nameDOM === "account") {
      ...
      for (request of requestRests)
        if (
          ...
          if (request === "forgotPassword") request = "forgot_password";

          scriptDOM = await bridge.promiseDOMHandler(
            `${originPath}/account/${request}.html`
          );
          break;
        } else
    ...
    if (nameDOM === "orderStatus" || nameDOM === "orderHistory") {
      ...
      scriptDOM = await bridge.promiseDOMHandler(
        `${originPath}/order/${nameDOM}.html`
      );
    }

    // error if script DOM is invalid
    if (!scriptDOM) throw new Error("scripDOM: " + scriptDOM);

    const title = scriptDOM.querySelector("title");
    const content = scriptDOM.getElementById("main-content");
    let placeInsert = Array.from(mainContainer.children).find(
