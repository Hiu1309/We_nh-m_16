...
function createDots() {
  ...
  const container = elementsObj
    .getNewsBlogs()
    .querySelector(".nav-tab-container");
  ...
  if (window.innerWidth <= breakpoint) dotCount = childInners.length;
    ...
    childInners.forEach((child) => {
      if (child.classList.contains("active")) temp++;
    });

    if (temp === 0) dotCount = 0;
    else dotCount = Math.ceil(childInners.length / temp);
  }
...
function setTimeFS(elementsObj) {
  ...
  if (fSTime) {
    ...
    if (!stringTime) stringTime = activeFlashSale();
  ...
  try {
    ...
    if (
      ...
      productContainer.innerHTML =
        '<div class="s-m-hidden font-size-26 font-bold">Không có sản phẩm trong phần này</div>';
